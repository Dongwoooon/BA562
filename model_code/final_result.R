...
cs_all <- cs_all %>%
  ...
  select(CUS_ID, GENDER, AGE_GROUP, GROUP)
...
save(temp,file="DW_please.Rdata")

load("rf_group_train_prob.Rdata")
load("xgb_group_train_prob.Rdata")

weight_prev_rf=mean(rf_group_train_prob[,2])
weight_prev_xgb=mean(xgb_group_train_prob[,2])

w_rf = weight_prev_rf/(weight_prev_rf+weight_prev_xgb)
w_xgb = weight_prev_xgb/(weight_prev_rf+weight_prev_xgb)


ensem_group_train <- w_rf*rf_group_train + w_xgb*xgb_group_train_en
